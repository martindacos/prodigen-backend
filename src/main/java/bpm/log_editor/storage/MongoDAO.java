...
import es.usc.citius.womine.model.*;
...
public class MongoDAO {
    ...
    public static String infrequentPatters(LogFile log, Integer m, Double threshold) {
        ...
        try {
            ...
            JavaPairRDD<String, Trace> ciParaleliced = SparkConnection.getContext().parallelizePairs(caseInstances);
            ...
            return FileTranslator.resultsToJSON(run1, true);
        } finally {
            cursor.close();
        }
    }
    ...
    public static String frequentPatters(LogFile log, Integer m, Double threshold) {
        String hnName = getName(log, m);
        ...
        try {
            ...
            //System.out.println("Son iguales? " + caseInstancesG.equals(caseInstances));
            List<TaskVO> tasks = individual.getTasks();
            for (TaskVO t : tasks) {
    ...
    public static void frequentPattersBack(Graph model, String hnName, String fp_threshold, JavaPairRDD<String, Trace> ciParaleliced
        ...
        for (Double threshold : thresholds) {
            ...
            Set<Pattern> run1 = womine.runSpark();
            FileTranslator.resultsPatternsToDot(model, run1, true, hnName, false, threshold, r);
        }
    }
    ...
    public static void infrequentPattersBack(Graph model, String hnName, String ip_threshold, JavaPairRDD<String, Trace> ciParaleliced
        ...
        for (Double threshold : thresholds) {
            IGraphMining womine = new InfrequentPatterns(model, ciParaleliced, threshold, settings, numInstances);
            Set<Pattern> run1 = womine.runSpark();
            FileTranslator.resultsPatternsToDot(model, run1, true, hnName, true, threshold, r);
        }
    }
    ...
    private static void generateBackGraphs(Graph graph, String hnName, ActivityStatisticsVO actStats, ArcStatisticsVO arcStats,
        ...
        if (c != null) {
            ...
            hnName = fName + "/" + timeStamp + "_" + hnName;
        ...
        FileTranslator.ActivityStatsToDot(graph, actStats, hnName, c, r);

        FileTranslator.ArcStatsToDot(graph, arcStats, hnName, c, r);
    ...
    private static void addBranchs(Hierarchy h, HashMap<String, List<String>> fValues) {
        ...
        //Add ROOT node
        Node root = new Node(0, 0, "ROOT", true, null, null, null, new LinkedHashMap<>(), null, 0l, 0l);
            ...
            for (int j = 0; j < filter_by.size(); j++) {
                ...
                while (z < filtros.size()) {
                    ...
                    for (String s : splitFilters) {
                        ...
                        if (split.size()==1 && split.get(0).equals("ALL")) {
                    ...
                    Node node = new Node(id, (j + 1), values.get(0), true, z, null, null, filter2, null, 0l, 0l);
