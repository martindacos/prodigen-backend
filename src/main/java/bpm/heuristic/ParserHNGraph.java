...
class ParserHNGraph {
    ...
    static String translatePaint(Graph graph, Boolean removeComplete, List<INode> nodesPattern, Set<Arc> arcsPattern,
                                 Integer i, String frequency, String hnName, Boolean infrequent, Double threshold) {
        String graphDot;

        graphDot = "";

        graphDot += HEAD;
        graphDot += LABEL + "Frequency: " + frequency + "\";";
        graphDot += "\n";
        graphDot += "\n";
    ...
    static String translateStats(Graph graph, Boolean removeComplete,
                                 HashMap<String, ArrayList<String>> nodesToPaint, Double chunksize, boolean time,
                                 List<String> orderColor, List<List<Object>> activityLongerShorterDuration) {
        String graphDot;
        ...
        for (int i = 0; i<orderColor.size(); i++) {
            if (time) {
                ...
                double v = chunksize * (i + 1);
                long v2 = (long) (chunksize * i);
    ...
    private static String writeNodesSpecific(HashMap<String, ArrayList<String>> nodesToPaint) {
        String nodesString;

        nodesString = "";
        nodesString += checkIsDummyNodePaint(S_DUMMY_TASK, OPACITY_NODE);
        nodesString += checkIsDummyNodePaint(E_DUMMY_TASK, OPACITY_NODE);
        for (Map.Entry<String, ArrayList<String>> entry : nodesToPaint.entrySet()) {
    ...
    public static String translateArcsStats(Graph graph, boolean removeComplete, HashMap<String, ArrayList<bpm.statistics.Arc>> arcsToPaint,
                                            Double chunksize, List<String> orderColor, boolean printNumber) {
        String graphDot;
    ...
    public static void translateArcsPrune(boolean removeComplete, String hnName, String name,
    ...
    private static String writeArcsPaint2(HashMap<String, ArrayList<bpm.statistics.Arc>> arcsToPaint, boolean number) {
        String nodesString;

        nodesString = "";
        for (Map.Entry<String, ArrayList<bpm.statistics.Arc>> entry : arcsToPaint.entrySet()) {
            String color = entry.getKey();
            ArrayList<bpm.statistics.Arc> value = entry.getValue();
    ...
    public static String translateArcsLoops(Graph graph, boolean removeComplete,
                                          List<Object> loops, List<Object> end_loops, List<Object> nodes, List<bpm.statistics.Arc> arcsF) {
        String graphDot;
        ...
        graphDot += writeArcsPaint2(graph, removeComplete, loops, end_loops, arcsF);
        graphDot += "\n";
        graphDot += "}";
    ...
    private static String writeArcsPaint2(Graph graph, Boolean removeComplete, List<Object> loops, List<Object> end_loops, List<bpm.statistics.Arc> arcsF) {
        String nameActualTask, nameActualTaskB, nodesString;
        ...
        for (Arc a : arcs) {
            ...
            if (loops.contains(myArc) || end_loops.contains(myArc)) {
                Double fre = 0d;
