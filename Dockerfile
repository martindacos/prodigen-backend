...
RUN cd /opt/prodigen && gradle build
...
CMD mongod --fork --logpath /var/log/mongodb/mongod.log && ./spark-2.1.0-bin-hadoop2.7/sbin/start-master.sh && ./spark-2.1.0-bin-hadoop2.7/sbin/start-slave.sh spark://master:6066 && ./spark-2.1.0-bin-hadoop2.7/bin/spark-submit /opt/prodigen/build/libs/prodigen-backend-0.1.0.jar